...
function handleAddEntry() {
    ...
    const sheetsPayload = {
        ...
        reportType: "Zero Student Report",
        reportTitle: "Zero Student Timetable Log - " + subjectVal + " in " + roomVal,
        ...
        division: branchVal,
        participants: 0,
        coordinators: facultyInitials,
        driveLink: "",
        status: "Pending"
    };

    // 3. Compile date display for email
        ...
        <body style="margin: 0; padding: 0; background-color: #f8fafc; color: #334155;">
            <div style="background-color: #f8fafc; padding: 40px 20px;">
                <div style="max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 16px; border: 1px solid #cbd5e1; overflow: hidden; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.03);">
                    ...
                    <div style="padding: 24px;">
                        <table style="width: 100%; border-collapse: collapse; font-size: 13px; color: #334155;">
                            ...
                            <tr style="border-bottom: 1px solid #e2e8f0;">
                                ...
                                <td style="padding: 10px 0; color: #0f172a;">${timeInVal} - ${timeOutVal}</td>
                            </tr>
                            <tr>
                    ...
                    <div style="background-color: #f8fafc; padding: 20px; border-top: 1px solid #cbd5e1; text-align: center; font-size: 12px; color: #94a3b8;">
                        <p style="margin: 0;">This notification log was submitted from the <a href="${window.location.href}" style="color: #c0392b; text-decoration: none; font-weight: 600;">GMIU Faculty Portal</a>.</p>
                        <p style="margin: 4px 0 0 0;">&copy; 2026 Department of Information Technology, GMIU.</p>
                    </div>
    ...
    if (typeof SHEETS_CONFIG !== 'undefined' && SHEETS_CONFIG.ENABLED && SHEETS_CONFIG.WEBAPP_URL) {
        sheetsPromise = fetch(SHEETS_CONFIG.WEBAPP_URL, {
            method: 'POST',
            mode: 'no-cors',
    ...
    const emailPromise = fetch('send-email.php', {
        ...
        body: JSON.stringify({
            to: facultyEmail,
            cc: ["adminit@gmiu.edu.in"],
            subject: `Zero Student Class Timetable Log — ${facultyInitials}`,
            html: emailHtml
        })
    }).then(res => res.json());
